...
import { drag, extent, forceCenter, forceCollide, forceLink, forceManyBody, forceRadial, forceSimulation, scalePow, scaleSymlog, select } from 'd3';
...
const ForceNetwork = ({ value }) => {
  ...
  const [visRef, isVisible] = useIsVisible(0.3);
  ...
  const chart = useCallback(() => {
    ...
    const legendData = [{ label: value === '1' ? 'In 2024' : 'In 2003', color: '#000', group: 'title' }];

    // Legend group
    const legend = svg.append('g').attr('class', 'legend').attr('transform', 'translate(40, 40)');
    ...
    const nodesData = nodes_2003;
    let linksData = edges_2003;
    ...
    // --- Simulation ---
    const centerForce = forceRadial(d => 10 / Math.sqrt(d.value), width / 2, height / 2).strength(0.07); // stronger radial pull
    if (!simulationRef.current) {
      simulationRef.current = forceSimulation(nodesData)
        ...
        .force(
          'charge',
          forceManyBody().strength(d => -250 / Math.sqrt(d.value))
        )
    ...
    // --- Links ---
    const allEdgeValues = [...edges_2003.map(d => d.value), ...edges_2024.map(d => d.value)];
    ...
    // Arrow at midpoint
    linkEnter.append('path').attr('class', 'arrow').attr('fill', '#666').attr('d', 'M-10,-4 L0,0 L-10,4 Z');
    ...
    linkUpdate.select('.arrow').transition().duration(300).attr('opacity', 1);
    ...
    const allValues = [...nodes_2003.map(d => d.value), ...nodes_2024.map(d => d.value)];
    ...
    nodeSel.exit().transition().duration(500).attr('r', 0).attr('opacity', 0).remove();
    ...
    labelSel.exit().transition().duration(500).attr('opacity', 0).remove();
    ...
    simulationRef.current.on('tick', () => {
      svg.selectAll('.link-group').each((d, i, nodes) => {
        ...
        // Update curved path
        select(nodes[i]).select('.link').attr('d', `M${sx},${sy} Q${cx},${cy} ${tx},${ty}`);
        ...
        select(nodes[i]).select('.arrow').attr('transform', `translate(${x},${y}) rotate(${angle})`);
      });
